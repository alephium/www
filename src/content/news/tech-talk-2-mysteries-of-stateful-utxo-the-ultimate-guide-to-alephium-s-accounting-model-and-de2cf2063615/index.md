...
When Satoshi invented Bitcoin 10 years ago, it was not designed for the dApps use case. It’s very natural that other developers subsequently came up with different models to get around this, like the extended UTXO (eUTXO) popularized by [Cardano](https://docs.cardano.org/learn/eutxo-explainer/) or [Ergo](https://dav009.medium.com/learning-ergo-101-blockchain-paradigm-eutxo-c90b0274cf5e), the [Nervos](https://medium.com/nervosnetwork/https-medium-com-nervosnetwork-cell-model-7323fca57571) cell model, and Alephium’s stateful UTXO (sUTXO) model.
...
Conversely, the account model (mainly popularized by Ethereum) gives developers and smart contracts broad access to all information stored in the state and much better, thereby significantly enhancing [expressiveness](https://en.wikipedia.org/wiki/Expressive_power_%28computer_science%29).
...
The eUTXO model is one of the most well-known UTXO variants. It has advanced the programmability of the UTXO model by allowing arbitrary logic in the lockup scripts of UTXOs. While it preserves the immutability of the classic UTXO model, it raises [challenges](https://liberlion.medium.com/concurrency-in-the-eutxo-model-is-not-a-problem-but-a-challenge-db4b395a8eda) when it comes to the concurrent execution of smart contract transactions.
...
**CW:** It’s been the evolution of an idea! In 2017 and 2018, I was laser-focused on how to scale an L1. That’s when I first conceived of the [Blockflow sharding algorithm](https://github.com/alephium/white-paper/blob/master/alephium.pdf). It was designed to be way simpler than the (ever-shifting) [scaling roadmap](https://members.delphidigital.io/reports/the-hitchhikers-guide-to-ethereum/) of Ethereum 2.0, and was exciting, so I got my hands dirty to build it.
...
In fact, in Alephium, there are several types of data contained in the state: UTXO sets (describing address balances), smart contract logic (the “programs”) and smart contracts data. All of these together describe the state of Alephium, and are stored in [Merkle Trees](https://ethereum.org/pt/developers/docs/data-structures-and-encoding/patricia-merkle-trie/).
