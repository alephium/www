...
Use it now: [https://bridge.alephium.org/](https://bridge.alephium.org/#/transfer)
...
This bridge is built on top of a [generic message-passing protocol](https://docs.wormhole.com/wormhole/) that enables communication of messages between blockchains. It routes and verifies the locking, minting and unlocking events from one chain to another, effectively exchanging messages between the chains. This ensures that the assets can’t be used simultaneously on both chains, maintaining the integrity of the bridging process.
...
**The core contracts** ([Alephium](https://explorer.alephium.org/addresses/23Fj7xr1pxWfYLixz3aBC3u5dUJVpAjXArbpiYWxeGjQT) and [Ethereum](https://etherscan.io/address/0x579a3bDE631c3d8068CbFE3dc45B0F14EC18dD43)) are the system’s heart. Guardians monitor these contracts, which are pivotal for cross-chain communication. These contracts are deployed on each chain.
...
Alephium provides a front end for the bridge (called the bridge UI). Inspired by [the Portal Bridge](https://www.portalbridge.com/), the wormhole bridge frontend, it is an easy-to-use and convenient interface to bridge both ways (see more below).
...
First, anyone can use the bridge directly via smart contracts ([Alephium](https://explorer.alephium.org/addresses/23Fj7xr1pxWfYLixz3aBC3u5dUJVpAjXArbpiYWxeGjQT) and [Ethereum](https://etherscan.io/address/0x579a3bDE631c3d8068CbFE3dc45B0F14EC18dD43)), without any kind of front-end interface. This will be very useful for dApps & other services that might want to use the bridge programmatically, with no human interaction.

Second, anyone can run its own front-end if they so wish. Hey, you can even [fork the bridge](https://github.com/alephium/wormhole-fork) (that is open source) and create your own guardian network!

Third, Alephium provides [its own interface](https://bridge.alephium.org/), a front-end to use the bridge in a simple and convenient way.
...
From Alephium to Ethereum, [ALPH](https://etherscan.io/token/0x590f820444fa3638e022776752c5eef34e2f89a6) is registered, and all tokens on this [token list](https://github.com/alephium/token-list) can be registered and available for bridging.
...
Furthermore, any token that is on [this list](https://tokenlists.org/token-list?url=tokens.1inch.eth) can be registered by anyone, using the token registration page [here](https://bridge.alephium.org/#/register) and subsequently bridged through the bridge UI. But it won’t appear as “verified” in the wallets. If you want the token to appear as verified, you’ll need to do a push request on the token list [here](https://github.com/alephium/token-list).
...
More details can be found [here](https://github.com/wormhole-foundation/wormhole/blob/main/whitepapers/0002_governance_messaging.md).
...
[As was](https://twitter.com/alephium/status/1579824159273541632) [detailed in](https://twitter.com/alephium/status/1582368148631695361) [multiple updates](https://twitter.com/alephium/status/1587440588692934656) [over the](https://twitter.com/alephium/status/1715361062823079937) [past year](https://twitter.com/alephium/status/1716858711493493013), the bridge has been patiently and relentlessly built, software brick by process brick. This is the first iteration and it will still evolve in the future. Core contributors are considering adding more chains to connect to other ecosystems, new bridge features (such as bridging NFTs), and depending on usage and value transferred, relayers and different guardians configurations might be examined.
