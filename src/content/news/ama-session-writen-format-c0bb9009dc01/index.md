...
With the UTXO model, the token becomes the first class citizen on Alephium, making it possible to retrieve a user’s all tokens and display them in the wallet. We also have a [repo called token-list](https://github.com/alephium/token-list) to store more metadata for tokens, such as logos, making the display more user-friendly. Currently, our extension wallet works nicely in this way.
...
A: In the near future, Alephium has some exciting launches planned. The core dev team has been developing and testing the d[ecentralized exchange (DEX)](/news/post/dex-prototype-live-on-testnet-bac5e7d095ce), bridge, and [NFT features](https://github.com/alephium/alephium-nft) for a while. Additionally, community developers have been contributing to these efforts, which is fantastic news.
...
**Q: What does it mean** [Schnorr Signatures](https://twitter.com/alephium/status/1638541074074578946) **support for the average user and dApps developers?**

A: Schnorr signatures are supported at two levels: wallet and VM.

Only the [extension wallet](/news/post/alephium-launches-browser-extension-wallet-706dfeda98f5) currently supports Schnorr signatures.   
