...
const MeshGradientEffect: FC<MeshGradientEffectProps> = ({ className }) => {
  ...
  const leftGradient = `radial-gradient(circle at 0% 50%, ${theme.palette2} 15%, ${theme.palette5} 25%, ${theme.palette1} 35%, ${theme.palette3} 40%, transparent 70%)`
  const rightGradient = `radial-gradient(circle at 100% 50%, ${theme.palette2} 15%, ${theme.palette5} 25%, ${theme.palette1} 35%, ${theme.palette3} 40%, transparent 70%)`

  return (
    <EddyBackgroundContainer className={className}>
      <GradientContainer
        style={{ backgroundImage: leftGradient, left: 0, backgroundPosition: '0% 50%', transformOrigin: '0% 50%' }}
        aria-hidden="true"
      />
      <GradientContainer
        style={{
...
const GradientContainer = styled.div`
  ...
  opacity: ${({ theme }) => (theme.name === 'dark' ? 0.4 : 0.5)};
